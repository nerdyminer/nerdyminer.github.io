...
- *La media esperada de tratamiento, para este contexto, es cercana a los 6500 tph*.
- *El próximo día observado "debiera" caer cerca de los 6500 tph*.
...
En este modelo, cuando hablamos de "global", no nos referimos a causalidad ni universalidad, sino a que no permitimos que la pendiente cambie por cluster, mes o línea. Esta elección de modelamiento controla la complejidad del modelo con 518 días observados, la que se estima debe ser razonablemente baja. Alternativas más sofisticadas podrían permitir pendientes variables por firma, pero añadirían parámetros e interacciones que necesitarían soporte y validación propios.
...
- Distribuciones asumidas para los efectos de las firmas geológicas y su escala: $u_{c}^{\text{raw}}\sim \mathcal{N}(0, 1)$ y $\sigma_{u}\sim \bar{\mathcal{N}}(180)$, donde $\bar{\mathcal{N}}$ representa una distribución normal truncada a valores positivos. El número $180$ representa una escala inicial para la desviación estándar de los efectos de las firmas geológicas, que puede ajustarse según la variabilidad observada en los datos.
...
$$
\eta_{k} \sim \mathcal{N} \left( 0,1 \right) \quad ;\quad \sigma_{z} \sim \bar{\mathcal{N}} \left( 120 \right) \quad ;\quad \gamma_{k} =\sigma_{z} \eta_{k}
$$
...
$$
\xi_{m} \sim \mathcal{N} \left( 0,1 \right) \quad ;\quad \sigma_{h} \sim \bar{\mathcal{N}} \left( 100 \right) \quad ;\quad \delta_{m} =\sigma_{h} \xi_{m}
$$
...
Supongamos que disponemos de un conjunto de datos completo $\mathcal{M}$ que contiene un total de $M$ meses de observaciones (con granularidad diaria), siendo $12< M\leq 24$ (es decir, $\mathcal{M}$ contiene entre 1 y 2 años de datos). Para cada mes $t$ en el rango $[13, M-2]$, se define un conjunto de entrenamiento $\mathcal{D}_t$ que contiene los datos de los 12 meses anteriores a $t$, es decir, $\mathcal{D}_t = \{y_{d}, \mathbf{x}_{d}\}_{d=t-12}^{t-1}$. A partir de este conjunto de entrenamiento, se ajusta el modelo Bayesiano y se generan predicciones para los dos meses siguientes, es decir, para el conjunto de prueba $\mathcal{T}_t = \{y_{d}, \mathbf{x}_{d}\}_{d=t}^{t+1}$.

En palabras menos rimbombantes, el modelo se entrena con un año móvil de datos y se prueba con los dos meses siguientes, avanzando el corte de entrenamiento y prueba en pasos de dos meses, a modo de *forecasting*. Este procedimiento se repite para cada mes $t$ en el rango $[13, M-2]$, generando un total de $N = M - 14$ conjuntos de entrenamiento y prueba. De esta manera, se obtiene un conjunto de predicciones para cada mes $t$, que se pueden comparar con las observaciones reales para evaluar el desempeño del modelo.
...
En la ejecución del procedimiento de *backtesting* final para este proyecto, se evaluaron un total de 169 días, generando predicciones para cada uno de ellos y comparándolas con las observaciones reales. Los resultados obtenidos para las métricas de desempeño fueron los siguientes:
...
| RMSE | 298 tph |
| MAE | 240 tph |
| Sesgo predictivo | +18.5 tph |
| Cobertura predictiva nominal al 90% | 91.7% |

El sesgo predictivo local corresponde, en términos relativos, a un 0.28% del nivel medio de tratamiento de molienda observado en la planta. Esto indica que, en promedio, las predicciones generadas por el modelo tienden a sobreestimar ligeramente el tratamiento observado, pero la magnitud de este sesgo es relativamente pequeña en comparación con el nivel medio de planta (modelo virtualmente *insesgado*). Sin embargo, es importante tener en cuenta que este sesgo puede variar localmente a lo largo del tiempo, y que los *pliegues* de validación pueden mostrar desviaciones tanto positivas como negativas.
