...
[Mínimos cuadrados]{.class-keyword}
[Máxima verosimilitud]{.class-keyword}
[Ecuaciones normales]{.class-keyword}
[Gradiente descendente]{.class-keyword}
[Gradiente estocástico]{.class-keyword}
[Mini-batch]{.class-keyword}
[Inferencia estadística]{.class-keyword}
...
**Ejemplo 5.1 – Una implementación desde cero del método de mínimos cuadrados:** La discusión anterior quizás fue un tanto densa, así que es justo comenzar a *entrenar los dedos* y entender cómo podría ser su implementación a nivel de código. En este sencillo ejercicio, crearemos una clase llamada `OLSRegression` que, siguiendo la filosofía de la API de <strong><font color='darkmagenta'>Scikit-Learn</font></strong>, implementará la técnica de estimación por mínimos cuadrados para construir un modelo de regresión lineal, dados una matriz de datos de entrada `X` y un vector de valores de respuesta `y`, ambos representados por arreglos de <strong><font color='darkmagenta'>Numpy</font></strong>. Recordemos que el vector de parámetros de la regresión, que llamaremos `w`, será calculado de manera directa por medio de la ecuación normal (5.12).
...
#| fig-cap: "Muestra lineal univariable perturbada con ruido Gaussiano para ilustrar la estimación por mínimos cuadrados."
...
#| fig-cap: "Recta de mínimos cuadrados ajustada a una variable explicativa, junto con la dispersión de las observaciones utilizadas para estimarla."
...
#| fig-cap: "Nube tridimensional simulada a partir de dos predictores y una respuesta lineal perturbada por ruido Gaussiano."
...
La función regresora ahora describe un plano de ecuación $\hat{\mathbf{y} } =\hat{b} +\hat{w}_{1} \mathbf{x}_{1} +\hat{w}_{2} \mathbf{x}_{2}$, siendo $\hat{b},\hat{w}_{1}$ y $\hat{w}_{2}$ las estimaciones realizadas para $b,w_{1}$ y $w_{2}$, respectivamente. Podemos visualizar fácilmente este plano en <strong><font color='darkmagenta'>Scikit-Learn</font></strong> mediante la incorporación de una grilla tridimensional limitada, en cada dimensión, por los valores extremos de cada una de las variables independientes que conforman a la matriz $\mathbf{X}$. De esta manera, tenemos:
...
#| fig-cap: "Plano de regresión estimado mediante mínimos cuadrados para dos variables explicativas y respuesta continua."
...
**Ejemplo 5.2 – El efecto de la tasa de aprendizaje:** La implementación de un modelo de regresión lineal con coeficientes estimados por medio del algoritmo de gradiente descendente de tipo batch (BGD) es muy sencilla de implementar en Python. Para ilustrarlo, crearemos una clase que mantenga la consistencia de la API de <strong><font color='darkmagenta'>Scikit-Learn</font></strong> para su aplicación, de la misma forma que lo hicimos para el caso de la estimación por mínimos cuadrados. En esta oportunidad, nuestra clase se llamará `BGDRegression`, para diferenciar cada implementación (y le agregaremos una barra de progreso):
...
#| fig-cap: "Muestra lineal simulada utilizada para seguir la convergencia del gradiente descendente por lotes."
# Visualizamos nuestro conjunto de datos.
fig, ax = plt.subplots(figsize=(9, 5))
...
#| fig-cap: "Recta estimada mediante gradiente descendente y ajuste resultante respecto de las observaciones utilizadas durante el entrenamiento."
...
#| fig-cap: "Rectas intermedias recorridas por el gradiente descendente por lotes hasta converger al ajuste lineal final."
...
#| fig-cap: "Recorrido de los parámetros de sesgo y pendiente durante la convergencia del gradiente descendente por lotes."
...
#| fig-cap: "Trayectoria estocástica de los parámetros del modelo al actualizar el gradiente con una observación por iteración."
# Construimos el espacio de parámetros completo.
...
#| fig-cap: "Recorrido de los parámetros durante la optimización por SGD con moméntum, desde la condición inicial hasta la vecindad del mínimo."
# Construimos el espacio de parámetros completo.
...
#| fig-cap: "Recorrido de los parámetros del modelo bajo actualizaciones por mini-lotes hasta estabilizarse cerca del óptimo."
# Construimos el espacio de parámetros completo.
...
#| fig-cap: "Recorridos paramétricos de las variantes de descenso por gradiente, comparados desde una inicialización común hasta su convergencia."
...
**Ejemplo 5.7 – Gradiente descendente estocástico:** La implementación del modelo de regresión lineal por medio del algoritmo de gradiente descendente estocástico (SGD) en <strong><font color='darkmagenta'>Scikit-Learn</font></strong> se realiza por medio de la clase `SGDRegressor`, la cual viene equipada con varios parámetros, entre los cuales consideramos:
...
- `intercept_`: Valor estimado para el parámetro de sesgo del modelo de regresión. Se trata de un arreglo de <strong><font color='darkmagenta'>Numpy</font></strong> de dimensión `(1,)`.
- `coef_`: Valores estimados para los parámetros del modelo de regresión. Este resultado se retorna en la forma de un arreglo de <strong><font color='darkmagenta'>Numpy</font></strong> de dimensión `(n_features)`, donde `n_features` es el número de variables independientes en nuestro conjunto de entrenamiento.
...
#| fig-cap: "Crecimiento del número de variables polinomiales resultantes según el grado y la cantidad de variables originales."
...
<strong><font color='darkmagenta'>Scikit-Learn</font></strong> nos provee de una implementación sencilla del modelo de regresión polinomial (completo) en la forma de un transformador de variables similar a los que ya hemos usado previamente en otros ejercicios, como `StandardScaler`. En este caso, dicho transformador es `PolynomialFeatures`, y funciona de manera similar a éste último, siguiendo la filosofía de escalabilidad del API de <strong><font color='darkmagenta'>Scikit-Learn</font></strong>. Algunos parámetros de importancia para este transformador son los siguientes:
...
#| fig-cap: "Muestra cuadrática simulada con ruido Gaussiano utilizada para estudiar el ajuste mediante características polinomiales."
# Visualizamos nuestro conjunto de datos.
fig, ax = plt.subplots(figsize=(9, 6))
...
#| fig-cap: "Ajuste polinomial de segundo grado sobre la muestra simulada; la curva recupera la tendencia cuadrática subyacente."
...
#| fig-cap: "Serie sintética formada por tres componentes sinusoidales de frecuencia conocida y ruido Gaussiano aditivo."
...
#| fig-cap: "Reconstrucción linealizada de la señal periódica frente a las observaciones ruidosas empleadas para estimar sus cambios de fase."
...
#| fig-cap: "Familia de señales obtenidas al explorar combinaciones de frecuencias, con el mejor ajuste resaltado sobre los datos de entrenamiento."
...
#| fig-cap: "Muestra sintética de diez observaciones, donde la escasez de datos hace visible la variabilidad alrededor de la relación generadora."
# Visualizamos nuestro conjunto de datos.
fig, ax = plt.subplots(figsize=(9, 5))
...
#| fig-cap: "Efecto del grado polinomial sobre la flexibilidad del ajuste: comparación entre modelos de grados 1, 3, 5 y 10."
...
<strong><font color='blue'>Definición 5.2 – Error medio absoluto</font></strong>: Consideremos el mismo contexto de la definición 5.1. Se define el **error medio absoluto** (o **MAE**, del inglés *mean absolute error*) de un modelo como
...
#| fig-cap: "Muestra sintética de cien observaciones, con una representación más estable de la tendencia poblacional subyacente."
# Y lo graficamos.
fig, ax = plt.subplots(figsize=(9, 5))
ax.scatter(X, y, color="dodgerblue")
ax.set_title(
...
#| fig-cap: "Realización alternativa de cien observaciones, utilizada para mostrar la variación muestral aun bajo un mismo proceso generador."
...
#| fig-cap: "Métricas de ajuste sobre entrenamiento para modelos polinomiales de complejidad creciente."
# Visualizamos los resultados.
...
#| fig-cap: "Métricas sobre datos de prueba que permiten contrastar la capacidad de generalización de los modelos polinomiales."
# Visualizamos los resultados.
fig, ax = plt.subplots(figsize=(9, 9))
(
    metrics_vals_test
