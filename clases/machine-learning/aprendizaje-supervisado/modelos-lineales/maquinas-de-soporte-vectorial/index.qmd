...
::: {.class-keywords}
[Máquinas de soporte vectorial]{.class-keyword}
[Hiperplanos]{.class-keyword}
[Margen máximo]{.class-keyword}
[Margen rígido]{.class-keyword}
[Margen blando]{.class-keyword}
[Pérdida hinge]{.class-keyword}
[Problema dual]{.class-keyword}
[Condiciones KKT]{.class-keyword}
[Funciones kernel]{.class-keyword}
...
Consideremos pues un problema de clasificación binaria representado por el conjunto de entrenamiento $\mathcal{D}$. Cada par $\left( \mathbf{x}_{i} ,y_{i}\right)$, donde $\mathbf{x}_{i}$ es una matriz fila que simplemente representa la instancia $i$-ésima asociada a la matriz de atributos $\mathbf{X}$, hace corresponder a un ejemplo observado $\mathbf{x}_{i}$ una etiqueta o clase $y_{i}$ (siendo $1\leq i\leq m$). Si $x_{i}\in \mathbb{R}^{2}$, el problema puede ilustrarse geométricamente en el plano $\mathbb{R}^{2}$ *pintando* cada punto $x_{i}$ de un color si $y_{i}=1$ (la clase positiva), y de otro color si $y_{i}=0$ (la clase negativa). Notemos que, en este ejemplo práctico, hemos asignado el valor $0$ a la clase negativa, fundamentalmente porque, en <strong><font color='darkmagenta'>Scikit-Learn</font></strong>, las clases en un problema de clasificación suelen empezar a numerarse precisamente desde el cero. Ejemplifiquemos esto haciendo uso de algo de código, tomando como base la función `make_blobs()` del módulo `sklearn.datasets`. Para ello, haremos uso de las importaciones usuales:
...
```{python}
# Suprimimos advertencias para mantener salidas limpias.
# NOTA: Nunca hacer esto en un entorno de producción, ya que
...
#| fig-cap: "Dos clases linealmente separables generadas como nubes Gaussianas para motivar el margen máximo de una SVM."
...
#| fig-cap: "Separación de las especies Setosa y Versicolor en el plano definido por el largo y el ancho de sus pétalos."
fig, ax = plt.subplots(figsize=(9, 5))
...
#| fig-cap: "Tres hiperplanos que separan correctamente las especies, pero dejan márgenes distintos frente a las observaciones más cercanas."
fig, ax = plt.subplots(figsize=(9, 5))
...
En general, el problema (7.20) sólo puede resolverse por medio de métodos iterativos, dependiendo del número de variables e iteraciones del mismo. En Python, es posible resolver un programa cuadrático de manera realtivamente sencilla utilizando varias librerías. En este ejercicio, introduciremos a <strong><font color='darkmagenta'>CVXPY</font></strong> (acrónimo de *convex Python*), que es una librería especializada en la resolución de problemas de optimización gobernados por funciones convexas, y viene equipada con una serie de métodos de solución o *solvers*. La belleza de esta librería radica en que permite mantener coherencia en la definición estándar de un programa cuadrático y en su compatibilidad con los arreglos de <strong><font color='mediumorchid'>Numpy</font></strong>, lo que añade versatilidad y rapidez.

En <strong><font color='darkmagenta'>CVXPY</font></strong>, los programas cuadráticos suelen expresarse separando las restricciones de desigualdad de aquellas que son únicamente de igualdad. Por lo tanto, modificaremos ligeramente la estructura del programa (7.20) como sigue:
...
Vamos pues a mostrar cómo resolver un programa cuadrático, usando <strong><font color='darkmagenta'>CVXPY</font></strong>, considerando el siguiente ejemplo:
...
En primer lugar, debemos reescribir el problema (7.22) en la *forma estándar* (7.21) a fin de poder resolverlo usando la librería <strong><font color='darkmagenta'>CVXPY</font></strong>. Como la función objetivo es $z(x_{1},x_{2})=5x_{1}^{2}+2x_{2}^{2}+3x_{1}-x_{2}+10$, tenemos que
...
Con esto, ya estamos listos para resolver el problema en <strong><font color='darkmagenta'>CVXPY</font></strong>. Primero definimos los arreglos (vectoriales y matriciales) calculados previamente haciendo uso de <strong><font color='darkmagenta'>Numpy</font></strong> como sigue:
...
La resolución de un programa cuadrático por medio de <strong><font color='darkmagenta'>CVXPY</font></strong> no reviste mayores dificultades, independiente del número de variables. En general, el mayor contratiempo será llevar el problema de interés a su forma estándar. Más adelante, como es la tónica en estos apuntes, construiremos una implementación desde cero de una máquina de soporte vectorial para la cual haremos uso de <strong><font color='darkmagenta'>CVXPY</font></strong>, una vez que demostremos que este modelo puede expresarse por medio de un programa cuadrático.
...
#| fig-cap: "Geometría del programa cuadrático resuelto con CVXPY, mostrando la función objetivo y las restricciones que determinan la solución factible."
...
#| fig-cap: "Pérdida hinge en función del margen firmado: sólo se penalizan observaciones dentro del margen o clasificadas incorrectamente."
...
**Ejemplo 7.2 – Visualización de una clasificación con márgenes blandos en <font color='darkmagenta'>Scikit-Learn</font>:** Vamos a ilustrar visualmente lo que ocurre al implementar un modelo con márgenes blandos haciendo uso, nuevamente, de la librería <strong><font color='darkmagenta'>Scikit-Learn</font></strong>. Este ejemplo versará, mayormente, sobre el uso del hiperparámetro $C$ que caracteriza al término de error en el problema (7.37) y su efecto a un nivel más práctico. A diferencia de las secciones anteriores, mostraremos la implementación de la máquina de soporte vectorial en <strong><font color='darkmagenta'>Scikit-Learn</font></strong> de manera previa a su construcción desde cero, ya que será útil para observar los efectos de la regularización inducida por la flexibilización de los márgenes de separación (aunque, por supuesto, construiremos nuestra implementación *from scratch* más adelante).

En <strong><font color='darkmagenta'>Scikit-Learn</font></strong>, las máquinas de soporte vectorial se encuentran disponibles en el módulo `sklearn.svm`, y vienen en dos *sabores*. Si nos enfocamos en los problemas de clasificación, las opciones son `SVC` y `LinearSVC`. Y como los nombres de estos objetos lo indican, la diferencia fundamental entre ambos es que uno de los modelos soporta únicamente fronteras de separación lineales entre las clases de interés (`LinearSVC`), mientras que el otro nos da la opción de construir fronteras de separación más generales por medio de la introducción de funciones kernel (`SVC`). Esto es algo que veremos más adelante, una vez que dejemos sentadas las bases del caso lineal.

Cuando creamos una máquina de soporte vectorial utilizando el objeto `LinearSVC`, podemos especificar varios argumentos para instanciar dicho modelo, entre los cuales se destaca el hiperparámetro `C`. En efecto, se trata de un **parámetro de regularización** que penaliza las pérdidas asociadas a las instancias que residen en el lado incorrecto del hiperplano de separación, haciendo uso de una norma determinada a su vez por el hiperparámetro `penalty`, y que naturalmente puede ser `"l1"` o `"l2"`, en referencia a las normas $\ell_{1}$ y $\ell_{2}$, respectivamente. Por defecto, <strong><font color='darkmagenta'>Scikit-Learn</font></strong> define este valor en `C=1.0` con una penalización con norma `"l2"`. Reducir `C` aumenta la regularización efectiva, porque disminuye el peso relativo de las violaciones de margen; aumentarlo hace lo contrario.

Es posible, además, definir el hiperparámetro booleano `dual`, el cual permite seleccionar el algoritmo que resolverá el problema de optimización inherente a nuestro modelo. En <strong><font color='darkmagenta'>Scikit-Learn</font></strong>, la elección de este algoritmo dependerá fundamentalmente de las dimensiones del conjunto de entrenamiento (basadas en el número de instancias y atributos del mismo, $m$ y $n$) y el tipo de penalización seleccionada para las instancias que residan en el lado incorrecto del hiperplano de separación. De este modo, pondremos `dual=False` cuando $m>n$. Siempre podemos forzar a <strong><font color='darkmagenta'>Scikit-Learn</font></strong> a tomar esta decisión definiendo este parámetro como `dual="auto"`.
...
<strong><font color='blue'>Definición 7.6 – Kernel:</font></strong> Sea $\mathcal{D}$ un conjunto no vacío arbitrario. Una función simétrica $K:\mathcal{D}\times \mathcal{D}\longrightarrow \mathbb{R}$ es llamada **kernel** (definido positivo) sobre $\mathcal{D}$ si la desigualdad
...
De esta manera, hemos transformado el problema (7.86) en un programa cuadrático estándar como el mostrado en la expresión (7.85). Esta formulación tiene la ventaja de ser relativamente sencilla de construir en Python, haciendo uso de <strong><font color='darkmagenta'>Numpy</font></strong> para manipular los arreglos de interés, y <strong><font color='darkmagenta'>CVXPY</font></strong> para resolver el programa cuadrático resultante de manera directa. Muchas implementaciones de las máquinas de soporte vectorial hacen uso de la expresión (7.85) para llevar a cabo el proceso de entrenamiento, y estos apuntes no serán la excepción, como veremos en un ejercicio más adelante.
...
Como antes, hemos transformado el problema primal (7.87) en un programa cuadrático con la estructura estándar (7.85). Con estas transformaciones, la construcción de una implementación propia de una máquina de soporte vectorial será mucho más sencilla, tomando como base una resolución del problema resultante por medio de la librería <strong><font color='darkmagenta'>CVXPY</font></strong>. La elección, en este caso, se reducirá a simplemente escoger qué problema planteamos en el proceso de entrenamiento.

**Ejemplo 7.5 – Una implementación desde cero de un modelo de clasificación con márgenes blandos:** Partiremos construyendo una implementación sencilla de una máquina de soporte vectorial apta para problemas con márgenes blandos (lineales), basada en <strong><font color='darkmagenta'>Numpy</font></strong>, que siga la filosofía de la API de <strong><font color='darkmagenta'>Scikit-Learn</font></strong> (vale decir, equiparemos a esta implementación con métodos tales como `fit()` y `predict()`). Llamaremos a esta implementación `LinearSoftMarginSVM`.

Previo a su construcción, y debido a que las máquinas de soporte vectorial requieren siempre de un proceso de escalamiento previo, nos aseguraremos de que nuestra clase `LinearSoftMarginSVM` sea compatible con cualquier otro objeto que pueda hacer uso de un modelo típico de <strong><font color='darkmagenta'>Scikit-Learn</font></strong>, lo que nos permitirá incorporar nuestra implementación en una pipeline de manera natural. Para ello, haremos uso de dos objetos especializados:

- `BaseEstimator`: Corresponde a una **clase de base** para todos los modelos de <strong><font color='darkmagenta'>Scikit-Learn</font></strong>. Con "base" nos referimos a que, cualquier objeto que herede sus métodos y atributos, disfrutará de propiedades deseables conforme la API de <strong><font color='darkmagenta'>Scikit-Learn</font></strong>. Por ejemplo, podrá ser utilizada por cualquier objeto especializado en la optimización de hiperparámetros (como `RandomSearchCV` y similares), dispondrá de representaciones de texto informativo en formatos idóneos (incluso HTML) en IDEs y terminales, podrá serializarse, podrá incorporarse en pipelines y podrá validarse con cualquier método de <strong><font color='darkmagenta'>Scikit-Learn</font></strong>.
- `ClassifierMixin`: Otra clase de base, que puede utilizarse para heredar algunas propiedades muy útiles de los modelos de clasificación provistos por <strong><font color='darkmagenta'>Scikit-Learn</font></strong>. Estas funcionalidades incluyen un método `score()` que permitirá calcular rápidamente la exactitud del modelo a construir, y reforzar que el método `fit()` (que debemos construir) exija siempre un arreglo unidimensional `y` respecto del cual entrenaremos nuestro modelo.

Además, validaremos la estructura de nuestra implementación usando algunas funciones utilitarias:

- `check_X_y()`: Función que acepta como parámetros a dos arreglos, denominados genéricamente `X` e `y`, que representan típicamente la matriz de diseño y el vector de valores observados para un conjunto de entrenamiento de interés, y que permite chequear la consistencia de estos arreglos conforme las reglas de <strong><font color='darkmagenta'>Scikit-Learn</font></strong> para la imputación de datos de entrenamiento en sus modelos. Esto fuerza a que `X` sea un arreglo bidimensional y a que `y` sea un arreglo unidimensional. Este método también chequea que `X` e `y` sean arreglos no vacíos y con entradas finitas.
- `check_array()`: Función que permite chequear que un arreglo cumpla con ciertas características importantes en cualquier método o clase de <strong><font color='darkmagenta'>Scikit-Learn</font></strong>, conforme las reglas de la API estimadora. En particular, tales reglas incluyen que el arreglo sea no vacío y que contenga entradas finitas.
...
#| fig-cap: "Conjunto MOONS con clases entrelazadas y frontera no lineal empleado para comparar distintas funciones kernel."
...
Por lo cual, una vez definidos los elementos estructurales correspondientes, sólo resta formular el programa cuadrático en <strong><font color='darkmagenta'>CVXPY</font></strong>. Una vez ajustado el modelo (esto es, resuelto el programa cuadrático), calculamos el término de sesgo del hiperplano de decisión en el espacio de atributos como
...
Probaremos nuestra implementación, al igual que el caso lineal, haciendo uso del conjunto de datos <strong><font color='forestgreen'>MOONS</font></strong>, usando un kernel de tipo radial Gaussiano, a fin de comparar las fronteras de separación resultantes y los correspondientes márgenes de decisión. Esto es más bien directo:
...
Hemos tenido un enorme cuidado en desarrollar, con el mayor detalle posible, la teoría subyacente a las máquinas de soporte vectorial. Hemos edificado dicha teoría de el caso más simple, correspondiente a un problema de márgenes rígidos, hasta el caso más general, donde hemos introducido el truco del kernel para construir fronteras de separación no lineales entre clases en un problema binario, permitiendo violaciones del márgen de separación (es decir, errores de clasificación). Hemos introducido de forma muy somera una implementación de las máquinas de soporte vectorial provista por <strong><font color='darkmagenta'>Scikit-Learn</font></strong>, y hemos construido nuestra implementación desde cero, a fin de probar nuestro aprendizaje. Por lo tanto, en esta sección, nos dedicaremos a desarrollar algunos ejemplos más sofisticados que nos permitan aprender a utilizar de manera eficiente las herramientas provistas por <strong><font color='darkmagenta'>Scikit-Learn</font></strong> para la construcción de este tipo de modelos.

**Ejemplo 7.7 – Una introducción a las SVMs en <font color='darkmagenta'>Scikit-Learn</font>:** Como ya habíamos comentado en un principio, en el ejemplo (7.2), las máquinas de soporte vectorial en <strong><font color='darkmagenta'>Scikit-Learn</font></strong> están disponibles en el módulo `sklearn.svm`. Para problemas de clasificación, existen tres tipos de implementación, dependiendo de nuestras necesidades:
...
- `probability`: Parámetro Booleano que permite activar (o no) la estimación de probabilidades de pertenencia a una clase. Esto debe activarse de manera previa a la aplicación del método `fit()`. Sin embargo, como comentamos previamente, esto ralentizará significativamente el entrenamiento del modelo, ya que *tras bambalinas* <strong><font color='darkmagenta'>Scikit-Learn</font></strong> aplicará un procedimiento de validación cruzada con 5 subconjuntos. Los resultados de `predict_proba()`, una vez realizado el entrenamiento, pueden no ser consistentes con los resultados del método `predict()`.
- `class_weight`: Parámetro que permite especificar los pesos de las clases de interés, lo que permite (con limitaciones) manejar problemas no balanceados. Es posible definer su valor en `class_weight="balanced"` para que <strong><font color='darkmagenta'>Scikit-Learn</font></strong> asigne un peso de forma automática a cada clase de manera inversamente proporcional a su frecuencia. También es posible especificar dichos pesos por medio de un diccionario de Python (mapeando cada clase a su peso). Por defecto, el valor de este parámetro es `class_weight=None`.

Algunos atributos de esta clase, además de los típicos:

- `support_`: Índices asociados a los vectores de soporte estimados por el modelo (una vez entrenado). Se trata de un arreglo de <strong><font color='darkmagenta'>Numpy</font></strong> de tamaño `(n_SV,)`, donde `n_SV` es el número de vectores de soporte.
- `support_vectors_`: Arreglo de <strong><font color='darkmagenta'>Numpy</font></strong> de tamaño `(n_SV, n)`, donde `n_SV` es el número de vectores de soporte y `n` es el número de variables independientes en la matriz de diseño `X`, en el cual se especifican los vectores de soporte estimados por el modelo.
- `n_support_`: Arreglo de <strong><font color='darkmagenta'>Numpy</font></strong> donde se especifica el número de vectores de soporte para cada clase.
...
#| fig-cap: "Fronteras y márgenes aprendidos sobre el dataset MOONS por SVMs de tipo lineal, polinomial, radial y sigmoidal."
...
Vamos a replicar el ejercicio anterior con otro conjunto de datos *de juguete*, denominado <strong><font color='forestgreen'>CIRCLES</font></strong>, y que resulta muy útil para probar la eficiencia de los modelos de clasificación binaria, al igual que el conjunto de datos <strong><font color='forestgreen'>MOONS</font></strong>. En este caso, dicho conjunto es también un dominio en $\mathbb{R}^{2}$, y está constituido por dos nubes de puntos caracterizadas por dos clases. La primera tiene la forma de un círculo y la segunda tiene la forma de un anillo que es concéntrico con respecto a dicho círculo:
...
#| fig-cap: "Conjunto CIRCLES formado por dos clases concéntricas, cuya separación requiere una transformación no lineal del espacio."
...
#| fig-cap: "Fronteras y márgenes de cuatro SVM sobre CIRCLES; los kernels no lineales recuperan la geometría concéntrica de las clases."
# Graficamos los márgenes de decisión resultantes en cada caso.
fig, ax = plt.subplots(figsize=(9, 7.5), nrows=2, ncols=2, sharex=True, sharey=True)
...
#| fig-cap: "Patrón espacial observado de daño en pilares, utilizado como referencia para evaluar las fronteras construidas por las SVMs."
...
#| fig-cap: "Distribución espacial de pilares clasificados como dañados o estables por SVMs de tipo lineal, polinomial, radial y sigmoidal."
...
#| fig-cap: "Desempeño de entrenamiento y validación de distintas SVMs al aumentar el tamaño muestral, con sus respectivas brechas de generalización."
...
La construcción de curvas de calibración en <strong><font color='darkmagenta'>Scikit-Learn</font></strong> es sencilla y suele realizarse por medio de la aplicación de dos objetos especializados: `CalibratedClassifierCV`, que permite implementar un proceso de validación cruzada (definido por el método `cv`) a fin de poder calibrar un modelo de clasificación para luego construir las correspondientes curvas de confiabilidad; y `CalibrationDisplay`, el cual es un objeto de tipo `display` propio de <strong><font color='darkmagenta'>Scikit-Learn</font></strong>. similar a otros como `LearningCurveDisplay`, y que permite construir directamente las curvas de interés. En particular, lo haremos directamente a partir de los modelos, usando el método `from_estimator()`:
...
#| fig-cap: "Curvas de confiabilidad e histogramas de probabilidad para evaluar la calibración de las cuatro SVMs aplicadas al daño de pilares."
# Graficamos nuestros resultados.
...
**Ejemplo 7.9 – Visualizando la banda $\epsilon$-insensible en un modelo de regresión:** Finalizaremos esta sección mostrando la implementación de la máquina de soporte vectorial para problemas de regresión (*SVR*, del inglés *support vector regressor*) provista por <strong><font color='darkmagenta'>Scikit-Learn</font></strong>. En rigor, tal implementación, denominada `SVR` difiere de la utilizada para abordar problemas de clasificación (`SVC`), fundamentalmente, en la introducción del hiperparámetro `epsilon`, el cual, como cabría esperar, permite definir el ancho de la banda de sensibilidad de nuestro modelo. De esta manera, algunos de los parámetros aceptados por este objeto son:
...
Como en el caso de la implementación propia para abordar problemas de clasificación (`SVC`), <strong><font color='darkmagenta'>Scikit-Learn</font></strong> también nos provee las mismas variantes para el caso de problemas de regresión, como `LinearSVR` y `NuSVR`, con las mismas características explicadas previamente para `LinearSVC` y `NuSVC`.
...
#| fig-cap: "Región de tolerancia y vectores de soporte de una regresión conforme una SVM lineal ajustada con $\\epsilon=0.5$."
...
#| fig-cap: "Funciones de decisión aprendidas por cuatro SVMs con distintas funciones kernel sobre el dominio unidimensional de nuestro ejemplo."
# Graficamos nuestros resultados.
titles = [
    "SVM con kernel lineal",
    "SVM con kernel polinomial cúbico",
