---
title: "Una introducción sencilla a los algoritmos de aprendizaje"
subtitle: "Riesgo empírico, parámetros, modelos probabilísticos y selección inicial de modelos"
...
::: {.class-keywords}
[Algoritmos de aprendizaje]{.class-keyword}
[Riesgo empírico]{.class-keyword}
[Función de pérdida]{.class-keyword}
[Parámetros]{.class-keyword}
[Modelos probabilísticos]{.class-keyword}
[Selección de modelos]{.class-keyword}
[Regularización]{.class-keyword}
[Estimadores]{.class-keyword}
[Sesgo]{.class-keyword}
[Varianza]{.class-keyword}
...
<strong><font color='blue'>Definición 1.1 – Algoritmo de aprendizaje (o algoritmo de *machine learning*):</font></strong> Decimos que un programa de computadora *aprende* desde la experiencia $E$, con respecto a algún conjunto de tareas o problemas $T$ y una determinada métrica de rendimiento $P$, si su desempeño en las tareas de $T$, medido por $P$, mejora con la experiencia $E$.
...
::: {.eq-scroll}
$$
P=\frac{1}{m} \sum^{m}_{i=1} \left( \hat{y}_{i} -y_{i}\right)^{2}
\tag{1.1}
$$
:::
...
Ya con todo lo repasado en matemáticas en la [sección de apuntes](/apuntes/), y con la (quizás no tan breve) introducción que hemos desarrollado en las líneas anteriores, estamos en posición de definir rigurosamente lo que significa *"aprender"*. Este verbo, para el caso de los algoritmos de machine learning, suele estribar en la estimación de parámetros con base en un conjunto de datos de entrenamiento.
...
### Funciones permisibles como hipótesis
...
::: {.eq-scroll}
$$
\  y_{i}\approx f\left( \mathbf{x}_{i} |\mathbf{w }^{\ast } \right)  \  ;\  \forall i=1,...,m
\tag{1.2}
$$
:::
...
**Ejemplo 1.1 – Regresión por mínimos cuadrados:** En la [entrada dedicada a la aplicación de <strong><font color='darkmagenta'>Numpy</font></strong> a la construcción de modelos de regresión y clasificación](/clases/data-analytics/introduccion-al-analisis-de-datos-en-python/computacion-vectorizada-y-arreglos-con-numpy/aplicaciones-en-modelos/), introducimos el modelo de regresión lineal por medio de un enfoque iterativo, haciendo uso del [algoritmo de gradiente descendente](/apuntes/calculo-incertidumbre-y-optimizacion/optimizacion-aplicada-al-aprendizaje-automatico/). En un contexto general, si disponemos de un conjunto de $m$ pares $(\mathbf{x}_{i},y_{i})$ ($1\leq i\leq m$), dicho modelo se construye en base a una combinación lineal de los valores de $\mathbf{x}_{i}\in \mathbb{R}^{n}$ y una colección de $n+1$ parámetros $\mathbf{w}=(w_{0},...,w_{n})^{\top}$. Solemos anexar un valor $x_{0}=1$ al vector $\mathbf{x}_{i}$ para todo $i=1,...,m$, a fin de empatar las dimensiones de $\mathbf{x}$ y $\mathbf{w}$. Por lo tanto, el predictor en este caso se escribe como

::: {.eq-scroll}
$$
f\left( \mathbf{x}_{i} |\mathbf{w } \right)  =\mathbf{w }^{\top } \mathbf{x}_{i} \  ;\  \forall i=1,...,m
\tag{1.3}
$$
:::
...
::: {.eq-scroll}
$$
f\left( \mathbf{x}_{i} |\mathbf{w } \right)  =w_{0} +\sum^{n}_{j=1} w_{j} x_{ij}\  ;\  \forall i=1,...,m
\tag{1.4}
$$
:::
...
::: {.eq-scroll}
$$
\mathbf{R}_{\mathrm{emp} } \left( f\  |\  \mathbf{X} ,\mathbf{y} \right)  =\frac{1}{m} \sum^{m}_{i=1} \ell \left( y_{i},\hat{y}_{i} \right)
\tag{1.5}
$$
:::
...
::: {.eq-scroll}
$$
\mathbf{R}_{\mathrm{emp} } \left( f\  |\  \mathbf{X} ,\mathbf{y} \right)  =\frac{1}{m} \sum^{m}_{i=1} \left( y_{i}-f\left( \mathbf{x}_{i} ,\mathbf{w } \right)  \right)^{2}
\tag{1.6}
$$
:::
...
::: {.eq-scroll}
$$
\min_{\mathbf{w } \in \mathbb{R}^{n+1} } \mathbf{R}_{\mathrm{emp} } \left( f\  |\  \mathbf{X} ,\mathbf{y} \right)  =\frac{1}{m} \sum^{m}_{i=1} \left( y_{i}-\mathbf{w }^{\top } \mathbf{x}_{i} \right)^{2}
\tag{1.7}
$$
:::
...
::: {.eq-scroll}
$$
\min_{\mathbf{w } \in \mathbb{R}^{n+1} } \mathbf{R}_{\mathrm{emp} } \left( f\  |\  \mathbf{X} ,\mathbf{y} \right)  =\frac{1}{m} \left\Vert \mathbf{y} -\mathbf{X} \mathbf{w } \right\Vert^{2}
\tag{1.8}
$$
:::
...
::: {.eq-scroll}
$$
\mathbf{R}_{\mathrm{real} } \left( f\right)  =\mathrm{E}_{\mathbf{x} ,y} \left[ \ell \left( y,f\left( \mathbf{x} \right)  \right)  \right]
\tag{1.9}
$$
:::
...
::: {.eq-scroll}
$$
\mathbf{R}_{\mathrm{test} } \left( f\  |\  \mathcal{D}_{\mathrm{test} } \right)
=\frac{1}{m_{\mathrm{test} }}\sum^{m_{\mathrm{test} }}_{i=1}
\ell\left(y^{\mathrm{test} }_{i}, f\left(\mathbf{x}^{\mathrm{test} }_{i}\right)\right)
\tag{1.10}
$$
:::
...
1. **¿Qué tan bien ajusta el predictor los datos usados para entrenarlo?** Esta pregunta se responde mediante el riesgo empírico de entrenamiento.
2. **¿Qué tan bien parece comportarse el predictor frente a datos que no participaron en su ajuste?** Esta pregunta se responde, de manera aproximada, mediante el riesgo de prueba.
...
### Regularización como control de capacidad
...
\min_{f\in\mathcal{H}}
\left[
\mathbf{R}_{\mathrm{emp}}\left(f\  |\  \mathbf{X},\mathbf{y}\right)
+\lambda \Omega(f)
\right]
\tag{1.11}
$$
...
\left[
\frac{1}{m}\sum^{m}_{i=1}\ell\left(y_i,f(\mathbf{x}_i|\mathbf{w})\right)
...
::: {.eq-scroll}
$$
\Omega_2(\mathbf{w})=\left\Vert \mathbf{w}\right\Vert_2^2
=\sum_{j=1}^{n}w_j^2,
\qquad
\Omega_1(\mathbf{w})=\left\Vert \mathbf{w}\right\Vert_1
...
::: {.eq-scroll}
$$
\Omega_{\mathrm{EN}}(\mathbf{w})
=\alpha \left\Vert \mathbf{w}\right\Vert_1
+(1-\alpha)\left\Vert \mathbf{w}\right\Vert_2^2,
...
1. Un modelo cuadrático fijo, que tiene baja capacidad y no cambia aunque aumente el número de datos.
2. Un modelo cuya capacidad se selecciona mediante validación, escogiendo el grado de un polinomio dentro de un conjunto de alternativas.

Para implementar este ejemplo usaremos <strong><font color='darkmagenta'>Scikit-Learn</font></strong>, una de las bibliotecas más importantes del ecosistema de aprendizaje automático en Python. Esta librería provee una API consistente para entrenar modelos, transformar datos, construir pipelines y evaluar desempeño. En las entradas siguientes será el eje central de nuestro recorrido práctico por modelos supervisados, selección de modelos, validación y preprocesamiento.
...
En primer lugar, definiremos una semilla aleatoria fija y construiremos algunas funciones auxiliares para generar datos, ajustar modelos polinomiales y seleccionar el mejor grado de polinomio mediante validación:
...
```{python}
# Funciones auxiliares para generar datos, ajustar modelos polinomiales
...
Ahora entrenamos los modelos para ilustrar su desempeño para tamaños cada vez más grandes del conjunto de datos de entrenamiento:
...
degree_mean, degree_std = mean_and_std(results["selected_degree"])
```
...
#| fig-cap: "Ilustración del principio no free lunch: Un modelo cuadrático fijo puede ser insuficiente para una función no lineal, mientras que la capacidad seleccionada por validación cambia con el número de datos disponibles. Este ejemplo se inspira en el maravilloso libro de Ian Goodfellow, Yoshua Bengio y Aaron Courville, *Deep Learning* (2016), aunque con una implementación y visualización propias."
#| fig-align: center
#| warning: false
#| message: false
...
::: {.eq-scroll}
$$
\mathrm{B}\left(\hat{f}_{\mathcal{D}}(\mathbf{x})\right)
=\mathrm{E}_{\mathcal{D}}\left[\hat{f}_{\mathcal{D}}(\mathbf{x})\right]
-f^{\ast}(\mathbf{x})
\tag{1.18}
$$
:::
...
::: {.eq-scroll}
$$
\hat{p}=\frac{1}{m}\sum^{m}_{i=1}X_i
\tag{1.19}
$$
:::
...
::: {.eq-scroll}
$$
\mathrm{E}\left[\hat{p}\right]
=\mathrm{E}\left[\frac{1}{m}\sum^{m}_{i=1}X_i\right]
=\frac{1}{m}\sum^{m}_{i=1}\mathrm{E}[X_i]
=p
\tag{1.20}
$$
:::
...
::: {.eq-scroll}
$$
\bar{X}=\frac{1}{m}\sum^{m}_{i=1}X_i
\tag{1.21}
$$
:::
...
$$
\hat{\sigma}^{2}_{m}
=\frac{1}{m}\sum^{m}_{i=1}\left(X_i-\bar{X}\right)^2
\tag{1.23}
$$
:::
...
::: {.eq-scroll}
$$
\mathrm{E}\left[\hat{\sigma}^{2}_{m}\right]
=\frac{m-1}{m}\sigma^2,
\qquad
\mathrm{B}\left(\hat{\sigma}^{2}_{m}\right)
=-\frac{1}{m}\sigma^2
\tag{1.24}
$$
:::
...
$$
S^2
=\frac{1}{m-1}\sum^{m}_{i=1}\left(X_i-\bar{X}\right)^2,
\qquad m\geq 2
\tag{1.25}
$$
:::
...
\mathrm{Var}(\hat{\theta})
=\mathrm{E}\left[
\left(\hat{\theta}-\mathrm{E}[\hat{\theta}]\right)^2
\right]
\tag{1.27}
$$
:::
...
$$
\mathrm{Var}(\hat{p})
=\mathrm{Var}\left(\frac{1}{m}\sum^{m}_{i=1}X_i\right)
=\frac{1}{m^2}\sum^{m}_{i=1}\mathrm{Var}(X_i)
=\frac{p(1-p)}{m}
\tag{1.29}
$$
:::
...
### Trade-off entre sesgo y varianza para la minimización del error cuadrático medio
...
::: {.eq-scroll}
$$
\mathrm{E}_{\mathcal{D},\varepsilon}
\left[
\left(y-\hat{f}_{\mathcal{D}}(\mathbf{x})\right)^2
\right]
=
\sigma^2
+\mathrm{B}\left(\hat{f}_{\mathcal{D}}(\mathbf{x})\right)^2
+\mathrm{Var}_{\mathcal{D}}\left(\hat{f}_{\mathcal{D}}(\mathbf{x})\right)
\tag{1.33}
$$
:::
...
$$
\lim_{m\to\infty}
P \left(\left|\hat{\theta}_m-\theta\right|>\varepsilon\right)=0
\tag{1.35}
$$
:::
...
$$
\mathcal{D}
=\left\{(\mathbf{x}_{i},y_i)\right\}_{i=1}^{m},
\qquad
...
=\prod_{i=1}^{m}p_{\boldsymbol{\theta}}(y_i|\mathbf{x}_{i})
\tag{1.38}
$$
:::
...
=\log L(\boldsymbol{\theta}|\mathcal{D})
=\sum_{i=1}^{m}\log p_{\boldsymbol{\theta}}(y_i|\mathbf{x}_{i})
\tag{1.40}
$$
:::
...
=-\frac{1}{m}\ell_m(\boldsymbol{\theta})
=-\frac{1}{m}\sum_{i=1}^{m}
\log p_{\boldsymbol{\theta}}(y_i|\mathbf{x}_{i})
\tag{1.42}
$$
:::
...
::: {.eq-scroll}
$$
\ell_{\mathrm{NLL}}(y_i,\mathbf{x}_i;\boldsymbol{\theta})
=-\log p_{\boldsymbol{\theta}}(y_i|\mathbf{x}_i)
\tag{1.44}
$$
:::
...
### Verosimilitud logarítmica condicional y error cuadrático medio
...
::: {.eq-scroll}
$$
y_i=f(\mathbf{x}_{i}|\mathbf{w})+\varepsilon_i,
\ ; \
...
::: {.eq-scroll}
$$
\mathrm{Var}(Y|\mathbf{x})
=\sigma^2
\ ; \
\text{para todo valor admisible de }\mathbf{x}
\tag{1.46}
$$
:::
...
::: {.eq-scroll}
$$
y_i|\mathbf{x}_i,\mathbf{w},\sigma^2
\sim
\mathcal{N}\left(f(\mathbf{x}_{i}|\mathbf{w}),\sigma^2\right)
\tag{1.47}
$$
:::
...
::: {.eq-scroll}
$$
p_{\boldsymbol{\theta}}(y_i|\mathbf{x}_{i})
=
\frac{1}{\sqrt{2\pi\sigma^2}}
\exp\left\{
-\frac{1}{2\sigma^2}
\left(y_i-f(\mathbf{x}_{i}|\mathbf{w})\right)^2
\right\}
\tag{1.48}
$$
:::
...
=h(y)
\exp\left\{
\boldsymbol{\eta}_{\boldsymbol{\theta}}(\mathbf{x})^{\top}\mathbf{T}(y)
-A\left(\boldsymbol{\eta}_{\boldsymbol{\theta}}(\mathbf{x})\right)
\right\}
\tag{1.52}
$$
:::
...
\left[
\log h(y_i)
+\boldsymbol{\eta}_{\boldsymbol{\theta}}(\mathbf{x}_i)^{\top}\mathbf{T}(y_i)
-A\left(\boldsymbol{\eta}_{\boldsymbol{\theta}}(\mathbf{x}_i)\right)
\right]
\tag{1.53}
$$
:::
...
::: {.eq-scroll}
$$
\mathbf{J}_{\boldsymbol{\theta}}(\mathbf{x}_i)
=
\frac{\partial \boldsymbol{\eta}_{\boldsymbol{\theta}}(\mathbf{x}_i)}
...
::: {.eq-scroll}
$$
\nabla_{\boldsymbol{\theta}}\ell_m(\boldsymbol{\theta})
=
\sum_{i=1}^{m}
\mathbf{J}_{\boldsymbol{\theta}}(\mathbf{x}_i)^{\top}
\left[
\mathbf{T}(y_i)
-\nabla A\left(\boldsymbol{\eta}_{\boldsymbol{\theta}}(\mathbf{x}_i)\right)
\right]
\tag{1.55}
$$
:::
...
::: {.eq-scroll}
$$
\hat{\boldsymbol{\psi}}_{\mathrm{MLE}}
=g\left(\hat{\boldsymbol{\theta}}_{\mathrm{MLE}}\right)
\tag{1.58}
$$
:::
...
\nabla_{\boldsymbol{\theta}}\log p_{\boldsymbol{\theta}}(Y|\mathbf{X})^{\top}
\right]
\tag{1.61}
$$
:::
...
\hat{\mathbf{w}}
\in
\underset{\mathbf{w}\in\mathcal{W}}{\operatorname{arg\,min}}
\frac{1}{m}
\sum_{i=1}^{m}
\ell\left(
y_i,
...
\left\{
\frac{1}{m_L}
\sum_{(\mathbf{x}_i,y_i)\in\mathcal{D}_L}
\ell_{\mathrm{sup}}
\left(
y_i,
...
### La maldición de la dimensionalidad y el aprendizaje profundo
...
::: {.eq-scroll}
$$
\mathbf{h}^{\left( 1 \right)} =\phi_{1} \left( \mathbf{x} \right) \  ;\  \mathbf{h}^{\left( 2 \right)} =\phi_{2} \left( \mathbf{h}^{\left( 1 \right)} \right) \  ;\  ...\  ;\  \mathbf{h}^{\left( L \right)} =\phi_{1} \left( \mathbf{h}^{\left( L-1 \right)} \right)
\tag{1.76}
$$
:::
...
r_t
=
\alpha_1(\text{throughput}_t)
-\alpha_2(\text{energía específica}_t)
-\alpha_3(\text{inestabilidad}_t)
-\alpha_4(\text{violaciones operacionales}_t)
\tag{1.79}
$$
:::
