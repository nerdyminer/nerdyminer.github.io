...
<strong><font color='blue'>Definición 6.1 – $\alpha$-lector:</font></strong> Sea $V$ un $\mathbb{K}$-espacio vectorial y $\alpha =\left\{ v_{1},...,v_{n}\right\}$ una base de $V$. Llamaremos $\alpha$-lector al conjunto $\alpha^{\ast } =\left\{ v^{\ast }_{1},...,v^{\ast }_{n}\right\}$, donde, para cada $k=1,...,n$, se tiene que
...
<strong><font color='blue'>Definición 6.6 – Forma cuadrática:</font></strong> Dada la matriz simétrica $\mathbf{A}\in \mathbb{R}^{n\times n}$, definimos la aplicación
...
<strong><font color='blue'>Definición 6.7 – Matriz (semi-)definida positiva (negativa):</font></strong> Sea $\mathbf{A}\in \mathbb{R}^{n\times n}$ una matriz simétrica. Diremos que:
...
<strong><font color='forestgreen'>Caso 1 – $\lambda_{1}\lambda_{2}\neq 0$:</font></strong> En este caso, completamos cuadrados en la ecuación (6.55), obteniendo
...
<strong><font color='forestgreen'>Caso 2 – $\lambda_{1}\lambda_{2}= 0$:</font></strong> Nuevamente tenemos dos subcasos distintos.
...
Esta ecuación representa una **parábola** con vértice en el punto $V=(-3,-2)$. Al compararla con la forma canónica $(y_{1}-k)^{2}=4p(x_{1}-h)$, obtenemos $4p=-\frac{1}{8}$ y, por consiguiente, $p=-\frac{1}{32}$. Su foco se encuentra entonces en $F=\left(-\frac{97}{32},-2\right)$. Dicha parábola está rotada en -45º con respecto al origen del sistema original $(x,y)$ (ya que $\cos(-45º)=\sqrt{2}/2$). De esta manera, volviendo a las variables originales, tenemos que
...
Por lo que, en el sistema original, el vértice está en el punto $V=\left(-\frac{5}{\sqrt{2}},-\frac{1}{\sqrt{2}}\right)$, mientras que el foco se localiza en $F=\left(-\frac{161}{32\sqrt{2}},-\frac{33}{32\sqrt{2}}\right)$. La orientación de la parábola y de su eje de simetría se ilustra en la @fig-parab.

```{python}
#| label: fig-parab
#| fig-cap: "Parábola descrita por la ecuación (6.67) después de transformar su forma canónica desde el sistema rotado al plano original. La línea segmentada muestra el eje de simetría; el vértice y el foco permiten reconocer su orientación y sentido de apertura."
#| echo: false
#| warning: false
#| fig-align: center
#| fig-width: 9
...
def rotated_to_original(x_1, y_1):
    """Transforma coordenadas rotadas al sistema original."""
    root_two = np.sqrt(2)
    return (x_1 + y_1) / root_two, (x_1 - y_1) / root_two

fig, ax = plt.subplots(figsize=(9, 6))

y_1 = np.linspace(-3.15, -0.85, 500)
x_1 = -3 - 8 * (y_1 + 2) ** 2
x, y = rotated_to_original(x_1, y_1)

vertex_rotated = (-3, -2)
focus_rotated = (-97 / 32, -2)
vertex = rotated_to_original(*vertex_rotated)
focus = rotated_to_original(*focus_rotated)

axis_x_1 = np.linspace(-14, 1, 100)
axis_y_1 = np.full_like(axis_x_1, -2)
axis_x, axis_y = rotated_to_original(axis_x_1, axis_y_1)

ax.plot(x, y, color="#2f6f9f", lw=2.8, label="Parábola")
ax.plot(
    axis_x,
    axis_y,
    color="#c0392b",
...
ax.scatter(*vertex, s=65, color="#202124", zorder=4, label="Vértice")
ax.scatter(
    *focus,
    s=70,
    marker="x",
...
ax.axhline(0, color="#5f6368", lw=0.9)
ax.axvline(0, color="#5f6368", lw=0.9)
ax.set_xlabel(r"$x$", fontsize=13, labelpad=10)
ax.set_ylabel(r"$y$", fontsize=13, labelpad=15, rotation=0)
ax.set_title("Orientación de la parábola en el sistema original", fontsize=13, fontweight="bold", pad=10)
...
La ecuación anterior describe, en el sistema $(u,v)$, una elipse con centro en el punto $P=\left( -\frac{1}{2} ,-2\right)$, con semiejes $\ell_{1} =\frac{\sqrt{5} }{2} \wedge \ell_{2} =\sqrt{5}$. La elipse se encuentra rotada 45º con respecto al origen del sistema original $(x,y)$. Por lo tanto, en dicho sistema, el centro de la elipse se localiza en el punto $P=\left(\frac{\sqrt{2}}{2}\left(-\frac{1}{2}-2\right),\frac{\sqrt{2}}{2}\left(-\frac{1}{2}+2\right)\right)=\left(-\frac{5\sqrt{2}}{4},\frac{3\sqrt{2}}{4}\right)$. La geometría resultante se observa en la @fig-elips.

```{python}
#| label: fig-elips
#| fig-cap: "Elipse obtenida para a=3 al transformar la ecuación reducida desde el sistema principal (u,v) al plano original. Las líneas segmentadas representan las direcciones de sus semiejes, determinadas por los autovectores de la forma cuadrática."
#| echo: false
#| warning: false
#| fig-align: center
#| fig-width: 9
#| fig-height: 6
def principal_to_original(u, v):
    """Transforma coordenadas principales al sistema original."""
    root_two = np.sqrt(2)
    return (u + v) / root_two, (u - v) / root_two

fig, ax = plt.subplots(figsize=(9, 6))

theta = np.linspace(0, 2 * np.pi, 600)
semi_axis_u = np.sqrt(5) / 2
semi_axis_v = np.sqrt(5)
center_u, center_v = -0.5, -2

u = center_u + semi_axis_u * np.cos(theta)
v = center_v + semi_axis_v * np.sin(theta)
x, y = principal_to_original(u, v)
center = principal_to_original(center_u, center_v)

u_axis = np.array(
    [center_u - semi_axis_u, center_u + semi_axis_u]
)
v_axis = np.full(2, center_v)
short_x, short_y = principal_to_original(u_axis, v_axis)

u_axis = np.full(2, center_u)
v_axis = np.array(
    [center_v - semi_axis_v, center_v + semi_axis_v]
)
long_x, long_y = principal_to_original(u_axis, v_axis)

ax.plot(x, y, color="#b23a8a", lw=2.8, label="Elipse")
ax.plot(
    short_x,
    short_y,
    color="#2f6f9f",
...
ax.plot(
    ...
    lw=1.8,
    ls="--",
    label=r"Semieje $\ell_2$",
)
ax.scatter(*center, s=65, color="#202124", zorder=4, label="Centro")
ax.annotate(
    r"$P$",
    center,
    xytext=(10, 10),
...
ax.set_xlabel(r"$x$", fontsize=13, labelpad=10)
ax.set_ylabel(r"$y$", fontsize=13, labelpad=15, rotation=0)
ax.set_title("Elipse y direcciones principales en el sistema original", fontsize=13, fontweight="bold", pad=10)
