...
#| fig-cap: "Crecimiento combinatorio del número de soluciones candidatas al aumentar la dimensión de un problema de optimización."
...
<strong><font color='blue'>Definición 7.2 – Representación:</font></strong> La **representación** de un individuo es la estructura de datos mediante la cual una solución candidata queda codificada dentro del algoritmo evolutivo.
...
<strong><font color='blue'>Definición 7.3 – Población:</font></strong> Una **población** en la generación $t$ es un conjunto finito de individuos
...
<strong><font color='blue'>Definición 7.4 – Aptitud:</font></strong> La **aptitud** de un individuo es un valor escalar que cuantifica la calidad de la solución que dicho individuo representa.
...
<strong><font color='blue'>Definición 7.5 – Selección:</font></strong> La **selección** es el mecanismo mediante el cual un algoritmo evolutivo asigna preferencia reproductiva o preferencia de supervivencia a ciertos individuos de la población, en función de su aptitud.
...
<strong><font color='blue'>Definición 7.6 – Operadores de variación:</font></strong> Los **operadores de variación** son reglas que, a partir de uno o más individuos existentes, generan nuevos individuos modificando su representación.
...
<strong><font color='blue'>Definición 7.7 – Reemplazo:</font></strong> El **reemplazo** es el mecanismo mediante el cual se construye la población $P^{(t+1)}$ a partir de la población actual $P^{(t)}$, de la descendencia $Q^{(t)}$, o de una combinación de ambas.
...
<strong><font color='blue'>Definición 7.8 – Criterio de término:</font></strong> El **criterio de término** es la condición lógica que determina el instante en que el algoritmo deja de iterar y devuelve la mejor solución encontrada.
...
```{python}
class InMemoryPillowWriter(PillowWriter):
    """Escribe una animación GIF en memoria, sin crear archivos intermedios."""
...
def animation_to_gif(animation, fps):
    """Convierte una animación de Matplotlib en una salida GIF de Jupyter."""
    buffer = BytesIO()
    animation.save(buffer, writer=InMemoryPillowWriter(fps=fps))
    encoded_gif = base64.b64encode(buffer.getvalue()).decode("ascii")
    return HTML(
        (
...
#| fig-cap: "Instancia euclidiana del problema del vendedor viajero con 60 ciudades distribuidas aleatoriamente en el plano."
...
#| fig-cap: "Circuito de menor longitud retenido por el algoritmo evolutivo; las aristas muestran el orden de visita y el cierre de la ruta."
...
#| fig-cap: "Longitud de la mejor ruta por generación y mejor valor acumulado; la estabilización de ambas curvas evidencia la convergencia del algoritmo evolutivo."
...
```{python}
#| label: fig-optimizacion-de-tipo-black-box-06
#| fig-cap: "Progresión del algoritmo evolutivo y su convergencia a una solución del TSP."
#| fig-align: center
...
# Convertimos la animación en una salida GIF sin crear un archivo intermedio.
animation_image = animation_to_gif(m, fps=10)
plt.close(fig)
animation_image
```
...
#| fig-cap: "Superficie multimodal de Ackley, con numerosos mínimos locales alrededor de una cuenca global estrecha en el origen."
...
#| fig-cap: "Curvas de nivel de Ackley; los anillos concéntricos permiten apreciar la sucesión de barreras locales que rodean el óptimo global."
...
A fin de maximizar la dificultad del testeo de la implementación que construiremos del algoritmo de evolución diferencial, definiremos la función de Ackley en Python para un arreglo de <strong><font color='darkmagenta'>Numpy</font></strong> de tamaño arbitrario:
...
Todas las posiciones que cumplen con un valor Booleano `True` son las que reemplazaremos por las mutaciones correspondientes. Para realizar este reemplazo, utilizamos la función `np.where()` de <strong><font color='darkmagenta'>Numpy</font></strong>:
...
#| fig-cap: "Distribución espacial de las ciudades de una instancia tridimensional del TSP antes de determinar el orden de visita."
...
A continuación aplicamos el algoritmo de evolución diferencial implementado en <strong><font color='darkmagenta'>SciPy</font></strong>. Como cada ciudad recibe una clave real, el espacio de búsqueda es simplemente el hipercubo $([0, 1])^{n}$:
...
<strong><font color='blue'>Definición 7.9 – Algoritmo de inteligencia por enjambre:</font></strong> Un **algoritmo de inteligencia por enjambre** es una metaheurística poblacional de optimización en la cual un conjunto de agentes explora un espacio de búsqueda siguiendo reglas locales de interacción, cooperación o comunicación, de modo que el comportamiento colectivo del sistema produce una mejora progresiva de las soluciones candidatas.
...
<strong><font color='blue'>Definición 7.11 – Enjambre:</font></strong> Un **enjambre** es el conjunto de agentes que participan simultáneamente en la exploración del espacio de búsqueda de un problema (usualmente de tipo *black-box*).
...
<strong><font color='blue'>Definición 7.12 – Regla de actualización:</font></strong> La **regla de actualización** de un agente es la ecuación o procedimiento que determina cómo cambia su estado en función de su estado actual, la información de otros agentes y, eventualmente, del entorno compartido.
...
<strong><font color='blue'>Definición 7.13 – Estigmergia:</font></strong> La **estigmergia** es un mecanismo de coordinación indirecta en el cual los agentes modifican un entorno compartido, y esas modificaciones influyen posteriormente en el comportamiento de otros agentes.
...
<strong><font color='blue'>Definición 7.14 – Criterio de término:</font></strong> El **criterio de término** es la condición lógica que determina cuándo el algoritmo deja de iterar y devuelve la mejor solución encontrada.
...
#| fig-cap: "Desplazamiento del mejor punto global de PSO sobre la superficie de Ackley hasta alcanzar la cuenca del óptimo."
...
![Evolución de la trayectoria del mejor punto global en PSO sobre la función de Ackley.](media/pso_ackley_2d.gif){#fig-optimizacion-de-tipo-black-box-15 fig-align="center" width="100%"}
...
#| fig-cap: "Superficie multimodal «caja de huevos», cuyas numerosas cuencas locales dificultan la búsqueda del mínimo global."
# Graficamos la superficie resultante de esta función.
fig = plt.figure(figsize=(9, 6))
ax = plt.axes(projection='3d')
ax.plot_surface(X, Y, Z, rstride=5, cstride=5, edgecolor="k", cmap='cool')
...
#| fig-cap: "Curvas de nivel de la función «caja de huevos», que revelan la distribución repetitiva de mínimos locales."
...
#| fig-cap: "Desplazamiento del enjambre y trayectoria del mejor punto global sobre la función multimodal conocida como caja de huevos."
# Visualización de la trayectoria del mejor punto global.
fig, ax = plt.subplots(figsize=(9, 6))
...
#| fig-cap: "Convergencia del mejor valor global encontrado por PSO sobre la función «caja de huevos»."
# Curva de convergencia del algoritmo PSO.
fig, ax = plt.subplots(figsize=(9, 5))
ax.plot(
    result["history_gbest_values"],
...
```{python}
#| label: fig-optimizacion-de-tipo-black-box-20
#| fig-cap: "Trayectoria del algoritmo PSO sobre la 'caja de huevos'."
#| fig-align: center
# Construimos la animación del enjambre.
fig, ax = plt.subplots(figsize=(9, 6))
...
# Convertimos la animación en una salida GIF sin crear un archivo intermedio.
animation_image = animation_to_gif(m, fps=8)
plt.close(fig)
animation_image
```
...
#| fig-cap: "Instancia tridimensional del TSP organizada en tres conglomerados; la separación espacial condiciona los enlaces intergrupo de la ruta."
...
#| fig-cap: "Solución encontrada para el TSP con ciudades agrupadas en clusters por medio del algoritmo ACO."
...
#| fig-cap: "Longitud de la mejor ruta de cada iteración y mejor solución acumulada durante ACO; la brecha entre ambas resume exploración y retención de élite."
...
<strong><font color='blue'>Definición 7.15 – Solución actual:</font></strong> La **solución actual** en la iteración $t$ es el punto $\mathbf{x}_{t}\in \mathcal{X}$ sobre el cual se encuentra actualmente situado el algoritmo durante su proceso de búsqueda.
...
<strong><font color='blue'>Definición 7.16 – Vecindario:</font></strong> Dada una solución $\mathbf{x}\in \mathcal{X}$, el **vecindario** de $\mathbf{x}$, denotado por $\mathcal{N}(\mathbf{x})$, es el conjunto de soluciones que pueden generarse a partir de $\mathbf{x}$ mediante una perturbación elemental del problema.
...
<strong><font color='blue'>Definición 7.17 – Perturbación:</font></strong> La **perturbación** es el operador que, dada una solución actual $\mathbf{x}$, genera una nueva solución candidata $\mathbf{y}$ dentro de su vecindario.
...
<strong><font color='blue'>Definición 7.18 – Criterio probabilístico de aceptación:</font></strong> El **criterio probabilístico de aceptación** es la regla que asigna una probabilidad a la transición $\mathbf{x}\to \mathbf{y}$ en función de la diferencia de calidad entre ambas soluciones y de ciertos parámetros de control del algoritmo.
...
<strong><font color='blue'>Definición 7.19 – Programa de control:</font></strong> El **programa de control** es la regla mediante la cual el parámetro principal de aceptación probabilística evoluciona a lo largo de las iteraciones.
...
Modelaremos el problema como un esquema de asignación promedio diaria de camiones a unidades de carguío, y usaremos la implementación del algoritmo de recocido dual provista por <strong><font color='darkmagenta'>Scipy</font></strong> para su resolución, la cual corresponde a la función `dual_annealing()` y cuya dependencia es el módulo `scipy.optimize`. Como se trata de un algoritmo de optimización continua, trabajaremos con una versión más *relajada* donde el número de camiones asignados a cada frente puede interpretarse como una **asignación promedio** durante el día.
