...
Partiendo en el punto inicial $\mathbf{x}_{0}=(3,1)^{\top}$, aplicaremos iterativamente el algoritmo de GD para obtener una secuencia de estimaciones que converjan al valor mínimo de $f$ y que es de nuestro interés. Para ilustrar gráficamente este procedimiento, lo implementaremos por medio de Python haciendo uso de las librerías que hemos aprendido previamente. Así pues, debido a que $f$ se ha definido por medio de una forma cuadrática, haremos uso de <strong><font color='darkmagenta'>Sympy</font></strong> para definir simbólicamente dicha función en Python y evaluarla de manera adecuada:
...
Notemos que nuestra implementación del algoritmo de GD requiere del gradiente de la función objetivo (es decir, su(s) derivada(s)). Debido a que hemos definido `f` por medio de una expresión de <strong><font color='darkmagenta'>Sympy</font></strong>, el cálculo de las derivadas parciales respectivas es ciertamente sencillo si aplicamos el método `diff()`. De esta manera:
...
#| fig-cap: "Observaciones simuladas alrededor de la relación lineal $y=2+3x$; la dispersión vertical proviene del ruido gaussiano añadido."
...
#| fig-cap: "Recta estimada mediante descenso de gradiente por lotes superpuesta a los datos; el ajuste recupera aproximadamente los parámetros usados en la simulación."
...
#| fig-cap: "Secuencia de rectas inducidas por las actualizaciones de BGD; las iteraciones se aproximan progresivamente al ajuste de mínimos cuadrados."
...
#| fig-cap: "Muestra de observaciones ruidosas generadas sobre un paraboloide hiperbólico para comparar optimizadores con escalas de curvatura diferentes."
...
#| fig-cap: "Evolución de las tasas efectivas por parámetro en RMSProp; el promedio móvil de gradientes cuadrados adapta cada dirección de forma independiente."
fig, ax = plt.subplots(figsize=(9, 5))
...
#| fig-cap: "Evolución del primer momento con corrección de sesgo en Adam, comparada con la estimación exponencial sin corregir durante las primeras iteraciones."
fig, ax = plt.subplots(figsize=(9, 5))
...
Podemos comprobar lo anterior graficando la función de entropía puntual para un rango completo de probabilidades tal que $0\leq u\leq 1$. En efecto, haciendo uso de <strong><font color='darkmagenta'>Matplotlib</font></strong>, tenemos que:
...
#| fig-cap: "Función de entropía binaria y cuerda entre dos probabilidades; la curva situada sobre la cuerda verifica visualmente su concavidad."
...
Este problema tiene un total de dos variables y cinco restricciones. En los problemas de este tipo, el espacio de soluciones factibles es el conjunto interior a la intersección de todas las rectas que representan las restricciones del problema (puesto que tales restricciones son lineales). Podemos graficar este conjunto aprovechando que éste se trata de un subconjunto de $\mathbb{R}^{2}$ y, para ello, haremos uso de <strong><font color='darkmagenta'>Matplotlib</font></strong>:
...
#| fig-cap: "Región factible determinada por la intersección de las restricciones lineales y ubicación de sus vértices candidatos al óptimo."
...
Vamos a resolver este programa cuadrático usando <strong><font color='darkmagenta'>CVXPY</font></strong>, una librería diseñada para formular problemas de optimización convexa de manera declarativa. Esto significa que, a diferencia de algoritmos como `scipy.optimize.minimize`, en <strong><font color='darkmagenta'>CVXPY</font></strong> no escribimos directamente una función objetivo como un `callable` arbitrario, sino que declaramos variables, función objetivo y restricciones usando objetos matemáticos propios de la librería.

::: {.callout-note}
## Nota

<strong><font color='darkmagenta'>CVXPY</font></strong> no es una librería tan famosa como <strong><font color='darkmagenta'>Scipy</font></strong> u otras más comunes en el análisis de datos con Python. Instalarla en nuestro entorno virtual es sencillo:
...
#| fig-cap: "Dos clases sintéticas separables en el plano, usadas para formular y resolver el hiperplano de margen máximo."
fig, ax = plt.subplots(figsize=(9, 6))
...
Formulemos ahora el problema usando <strong><font color='darkmagenta'>CVXPY</font></strong>. Esta es la parte importante, ya que esta librería exige declarar el problema en términos de variables, objetivo y restricciones:
...
Ahora resolvemos el problema. En general, <strong><font color='darkmagenta'>CVXPY</font></strong> dispondrá por defecto de varios algoritmos de solución apropiados para cada tipo de problema, siendo `OSQP` una elección popular y que fue desarrollado precisamente para ser implementado en esta librería:
...
Además, el ejemplo ilustra muy bien la lógica de <strong><font color='darkmagenta'>CVXPY</font></strong>. En esta librería no basta con entregar una función objetivo cualquiera. Debemos plantear el problema de manera estructurada:
...
#| fig-cap: "Coincidencia entre la conjugada de Fenchel calculada por maximización numérica y su expresión analítica para una función cuadrática."
