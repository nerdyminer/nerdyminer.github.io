...
lang: es
header-includes: |
  <link rel="canonical" href="https://nerdyminer.github.io/articulos/modelos-lane-y-vickers/">
...
<nav class="article-language-switch" aria-label="Idioma del artículo">
  <span>Idioma</span>
  <a class="is-active" href="/articulos/modelos-lane-y-vickers/" lang="es" hreflang="es" aria-current="page">ES</a>
  <a href="/en/articles/cutoff-grades-lane-vickers/" lang="en" hreflang="en">EN</a>
</nav>
```
...
#| fig-cap: "Flujos de caja, valor acumulado y diferencias incrementales entre las estrategias de ley fija e intertemporal. El panel inferior hace visible una mejora que queda oculta por la escala de los resultados absolutos."
fig, ax = plt.subplots(3, 1, figsize=(9, 9), sharex=True)

for schedule, label, color in [
    (static_schedule, "Ley fija", "darkgray"),
    (dynamic_schedule, "Política intertemporal", "dodgerblue"),
]:
    ax[0].plot(
        schedule["period"],
...
# Calculamos el aporte incremental de la política intertemporal respecto de la ley fija.
cash_flow_delta = (
...
# Las barras muestran en qué período se crea o se difiere valor sin ocultar su signo.
...
ax[2].bar(
    ...
    color=delta_colors,
    alpha=0.78,
    label=r"$\Delta$ flujo de caja",
)
ax[2].axhline(0, color="dimgray", lw=0.9)

# Una segunda escala permite seguir el valor incremental descontado y acumulado.
...
ax[2].set_xlabel("Período", fontsize=11, labelpad=10)
ax[2].set_ylabel(r"$\Delta$ flujo de caja (MUSD)", fontsize=11, labelpad=10)
ax_npv_delta.set_ylabel(
    r"$\Delta$ VAN acumulado (MUSD)",
    fontsize=11,
    labelpad=10,
    color="dodgerblue",
)
...
# Reunimos ambas escalas del panel incremental en una sola leyenda.
...
Las dos curvas superiores casi no pueden distinguirse porque la diferencia económica es diminuta respecto de la escala absoluta del proyecto. La serie gris sí está presente: Queda debajo de la línea magenta porque la política intertemporal se dibuja en segundo lugar y entrega flujos prácticamente iguales. Esto no significa que ambas optimizaciones hayan encontrado exactamente la misma secuencia de leyes, sino que, para este inventario discretizado, gran parte de esas variaciones no cambia el destino efectivo de las parcelas.

El panel inferior cambia deliberadamente la pregunta. En vez de volver a mostrar cuánto vale cada estrategia, representa cuánto agrega la política intertemporal sobre la ley fija en cada período. Las barras corresponden a la diferencia de flujo de caja, $\Delta CF_t=CF_t^{\mathrm{intertemporal}}-CF_t^{\mathrm{fija}}$: Una barra positiva indica creación o adelanto de caja respecto de la referencia, mientras que una barra negativa indica valor diferido o un costo incremental en ese período. La línea muestra la suma acumulada de esas diferencias después de descontarlas, es decir, la trayectoria de $\Delta VAN$.

Esta ampliación también evita una lectura engañosa. Una mejora visible en el panel incremental puede seguir siendo económicamente pequeña al compararla con los más de 600 MUSD del VAN total. Su utilidad no consiste en engrandecer el resultado, sino en localizar cuándo aparece: En esta realización, la mejora se concentra en los períodos 3 y 4; luego, el $\Delta VAN$ permanece prácticamente constante y termina cerca de 0,063 MUSD. El resultado confirma que el despacho por NSR ya captura casi todo el valor disponible y que la optimización temporal actúa únicamente sobre un margen muy estrecho.
