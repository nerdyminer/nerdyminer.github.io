...
```{mermaid}
%%| label: fig-grafopit
%%| fig-cap: "Construcción conceptual de la red dirigida para el problema de pit final. Los arcos económicos conectan la fuente o el sumidero según el signo del beneficio; los arcos segmentados de capacidad M imponen las precedencias de extracción; y el corte mínimo separa los bloques extraídos de aquellos que permanecen fuera del diseño."

flowchart TB
  subgraph ECONOMY["01 · El valor económico define las capacidades"]
    direction LR
    S(("Fuente<br/>s"))
    POS["Bloque rentable<br/>w > 0"]
    NEG["Bloque con costo<br/>w < 0"]
    T(("Sumidero<br/>t"))
  ...
  subgraph GEOMETRY["02 · El talud de 45° impone la precedencia"]
    direction TB
    B["Bloque de interés<br/>b(i,j)"]

    subgraph PREDECESSORS["Bloques superiores que deben extraerse primero"]
      direction LR
      LEFT["b(i−1,j−1)"]
      CENTRE["b(i−1,j)"]
      RIGHT["b(i−1,j+1)"]
  ...
  POS --> NETWORK["Red s–t completa<br/>ambas reglas se repiten<br/>para cada bloque"]
  B --> NETWORK

  NETWORK --> CUT["Corte mínimo<br/>(S,T)"]
  CUT --> EXTRACTED["S sin s<br/>bloques extraídos"]
  CUT --> REJECTED["T sin t<br/>bloques no extraídos"]

  classDef terminal fill:#315F86,stroke:#8CC8F4,stroke-width:2px,color:#FFFFFF
  classDef positive fill:#285B3A,stroke:#8DD19A,stroke-width:2px,color:#FFFFFF
  classDef negative fill:#6A2D2B,stroke:#F09A91,stroke-width:2px,color:#FFFFFF
  classDef block fill:#48334F,stroke:#D5A1DD,stroke-width:2px,color:#FFFFFF
  classDef predecessor fill:#233F49,stroke:#74C6D3,stroke-width:2px,color:#FFFFFF
  classDef process fill:#5A4026,stroke:#F1B45B,stroke-width:2px,color:#FFFFFF
  classDef selected fill:#36542C,stroke:#A7D98B,stroke-width:2px,color:#FFFFFF
  classDef rejected fill:#4B5156,stroke:#BFC8CE,stroke-width:2px,color:#FFFFFF

  class S,T terminal
  class POS positive
  class NEG negative
  class B block
  class LEFT,CENTRE,RIGHT predecessor
  class NETWORK,CUT process
  class EXTRACTED selected
  class REJECTED rejected

  style ECONOMY fill:#F5F8FA,stroke:#94AFC1,stroke-width:1px,color:#253D4A
  style GEOMETRY fill:#F8F3F8,stroke:#B998B5,stroke-width:1px,color:#422F40
  style PREDECESSORS fill:#EEF6F7,stroke:#94BDC2,stroke-width:1px,color:#294248

  linkStyle default stroke:#647B83,stroke-width:1.7px
```
