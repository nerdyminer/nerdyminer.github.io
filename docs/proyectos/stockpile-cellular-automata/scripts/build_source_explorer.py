...
.app{{display:grid;grid-template-columns:310px 1fr;height:100vh}}aside{{background:var(--panel);border-right:1px solid var(--line);display:flex;flex-direction:column;min-width:0}}
...
input{{width:100%;padding:9px 11px;border:1px solid var(--line);border-radius:7px}}nav{{overflow:auto;padding:8px}}
...
main{{min-width:0;display:flex;flex-direction:column}}header{{padding:16px 20px;background:var(--panel);border-bottom:1px solid var(--line);display:flex;justify-content:space-between;gap:18px;align-items:center}}.meta{{min-width:0}}.eyebrow{{color:var(--accent);font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.09em}}h2{{margin:2px 0;font:700 18px/1.2 Georgia,serif}}.description{{color:var(--muted)}}.actions{{display:flex;gap:8px}}button{{padding:8px 11px;border:1px solid var(--line);border-radius:7px;background:#fff;cursor:pointer}}button:disabled{{opacity:.35}}
.code-wrap{{overflow:auto;flex:1;background:var(--code)}}pre{{margin:0;padding:22px;min-width:max-content;color:var(--code-ink);font:12.5px/1.55 ui-monospace,SFMono-Regular,Menlo,monospace;tab-size:4;white-space:pre}}.counter{{font-variant-numeric:tabular-nums;color:var(--muted);white-space:nowrap}}
@media(max-width:780px){{.app{{grid-template-columns:1fr;height:auto}}aside{{max-height:42vh;border-right:0;border-bottom:1px solid var(--line)}}main{{height:100vh}}header{{align-items:flex-start;flex-direction:column}}}}
</style></head><body><div class="app"><aside><div class="aside-head"><h1>Stockpile CA</h1><p>Lectura guiada del repositorio público.</p><input id="search" type="search" placeholder="Filtrar archivos…"></div><nav id="steps"></nav></aside>
<main><header><div class="meta"><div id="section" class="eyebrow"></div><h2 id="path"></h2><div id="description" class="description"></div></div><div class="actions"><span id="counter" class="counter"></span><button id="previous" aria-label="Anterior">←</button><button id="next" aria-label="Siguiente">→</button><button id="copy">Copiar</button></div></header><div class="code-wrap"><pre><code id="source"></code></pre></div></main></div>
